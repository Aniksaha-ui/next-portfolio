...
import { TESTIMONIAL, TESTIMONIAL_DETAILS, TRAINING, TRAINING_DESCRIPTION } from "@/helpers/constants/pageConst";
import SingleTraining from "./SingleTraining";

const Training = () => {
  const [trainingData, setTraining] = useState<Testimonial[]>([]);
    ...
    const fetchTraning = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/users/training`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <section className="dark:bg-bg-color-dark bg-gray-light relative z-10 py-16 md:py-20 lg:py-28">
      <div className="container">
        <SectionTitle
          title={TRAINING}
          paragraph={TRAINING_DESCRIPTION}
          center
        />
